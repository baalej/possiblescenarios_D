...
function displayImages (identifier) {
	...
	if (selectedTags.length != 0 || selectedTags != null) {
		...
		imageSize = (smallest / imagesDisplayed) * galleryWidth;
		if (imageSize < 56) {
			imageSize = 56;
		}
		...
		for (let i = 0; i < projectResources.length; i++) {
			if (projectResources[i].classList.contains('text')) {
				projectResources[i].style.width = imageSize + 'px';
				projectResources[i].style.height = (projectResources[i].clientWidth) * 1.41 + 'px';

				let currentFontSize = window.getComputedStyle(projectResources[i]).fontSize;
				currentFontSize = currentFontSize.slice(0, -2);
				currentFontSize = parseInt(currentFontSize);

				let width = window.innerWidth;
				if (width < 640) {
					projectResources[i].style.fontSize = (projectResources[i].clientWidth) * 0.032 + 'px';
				} else {
					projectResources[i].style.fontSize = (projectResources[i].clientWidth) * 0.017 + 'px';
				}
			} else {
				projectResources[i].style.width = imageSize + 'px';
			}
		...
		for (let i = 0; i < projectResources.length; i++) {
			if (projectResources[i].classList.contains('text')) {
				projectResources[i].style.width = '56px';
				projectResources[i].style.height = (projectResources[i].clientWidth) * 1.41 + 'px';
			} else {
				projectResources[i].style.width = '56px';
			}
